...
2. From project root:
   ...
   npm run db:migrate
   ```
3. Migrations run in order: `schema.sql`, `schema-guardian-email.sql`, `schema-v2.sql`, `schema-iot-health.sql`, `schema-audit.sql`, `schema-optional-tables.sql` (classrooms, subjects), `schema-classrooms-subjects-fk.sql`, `schema-lab-computers.sql`. Each is idempotent where possible (`IF NOT EXISTS`).
